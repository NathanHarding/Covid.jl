...
quar_dates <- c("2020-02-04")
quar_days <-c(1,2,3,4,5,6,7)
quar_compl <-c(0.9,0.8,0.7,0.6,0.5,0.4,0.3)
...
for (i in 1:length(quar_dates))
{
  quarantine<-paste0(quarantine,"\n",indent,quar_dates[i],
                  ": {awaiting_test_result: {days: ",quar_days[7*i-6],
                  ", compliance: ", quar_compl[7*i-6],
                  "}, tested_positive: {days: ",quar_days[7*i-5],
                  ", compliance: ", quar_compl[7*i-5],
                  "}, case_contacts: {days: ",quar_days[7*i-4],
                  ", compliance: ", quar_compl[7*i-4],
                  "}, workplace: {days: ",quar_days[7*i-3],
                  ", compliance: ", quar_compl[7*i-3],
                  "}, school: {days: ",quar_days[7*i-2],
                  ", compliance: ", quar_compl[7*i-2],
                  "}, community: {days: ",quar_days[7*i-1],
                  ", compliance: ", quar_compl[7*i-1],
                  "}, social: {days: ",quar_days[7*i],
                  ", compliance: ", quar_compl[7*i],"}}")
}
config = paste(preamble,forcing,distancing,testing,tracing,quarantine)
write(config,"../test/dir_test/config.yml")
